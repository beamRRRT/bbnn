...
banner() {
    if [[ $(command -v figlet) ]]; then
        echo -e "${BLUE}"
        figlet -f standard "BEAMANC-PHISH"
        echo -e "${BLUE}                                ${WHITE}${__version__}\n"
        echo -e "${GREEN}[${WHITE}-${GREEN}]${CYAN} lnwper-Thanwisut (lnwper)${WHITE}"
    else
        cat <<- EOF
        ${BLUE}  ____  _____    _    __  __    _    _   _  ____ 
        ${BLUE} | __ )| ____|  / \  |  \/  |  / \  | \ | |/ ___|    
        ${BLUE} |  _ \|  _|   / _ \ | |\/| | / _ \ |  \| | |     
        ${BLUE} | |_) | |___ / ___ \| |  | |/ ___ \| |\  | |  
        ${BLUE} |____/|_____/_/   \_\_|  |_/_/   \_\_| \_|\____| 
        ${BLUE}   
        ${BLUE}   
        ${BLUE}                                 ${WHITE}${__version__}

        ${GREEN}[${WHITE}-${GREEN}]${CYAN} InwBeam (lnwBEAM)${WHITE}
EOF
    fi
}
...
about() {
    { clear; banner; echo; }
    cat <<- EOF
        ${GREEN} Author   ${RED}:  ${ORANGE}Thanawit
        ${GREEN} Github   ${RED}:  ${CYAN}https://github.com/beamRRRT
